...
--select employees that started joB 7 years ago
SELECT EMP_ID, F_NAME, DEP_ID, JOBS_ID, START_DATE
FROM EMPLOYEE E
INNER JOIN JOB_HISTORY JH
ON E.EMP_ID = JH.EMPL_ID
--WHERE GETDATE() > YEAR('2016/01/01')
WHERE YEAR(START_DATE) > (YEAR(GETDATE()) - 7)
...
-- ON E.EMP_ID = JH.EMPL_ID
--WHERE GETDATE() > YEAR('2016/01/01')
WHERE E.EMP_ID = JH.EMPL_ID AND
YEAR(START_DATE) > (YEAR(GETDATE()) - 7)
